...
import { IUser, ICredit, IDebit } from "../utils/interface";
...
export default class AdminDebitController {
  /**
     ...
     */
  static async sendMoney(req: Request, res: Response, next: NextFunction): Promise<object> {
    try {
      const { id } = req.user;
      ...
      const transaction: IDebit = await db("debits").insert({
        amount,
        type: "transfer",
        owner: receiver.id,
        sender: id,
  ...
  /**
     ...
     * @returns {object} Success message
     */
  static async getDebitTransactions(req: Request, res: Response, next: NextFunction): Promise<object> {
    try {
      const { id } = req.user
  ...
  /**
     ...
     * @returns {object} Success message
     */
  static async getDebitTransactionById(req: Request, res: Response, next: NextFunction): Promise<object> {
    try {
  ...
  /**
     ...
     * @returns {object} Success message
     */
  static async deleteDebitTransaction(req: Request, res: Response, next: NextFunction): Promise<object> {
    try {
      const { debitId } = req.params;
      const { id } = req.user;
      const transaction = await db("debits").first().where({ id: debitId, owner: id});
  ...
  /**
     ...
     * @returns {object} Success message
     */
  static async requestWithdrawal(req: Request, res: Response, next: NextFunction): Promise<object> {
    try {
      const { id } = req.user;
      ...
      if (!account) return errorResponse(res, 404, "Account does not exist kindly select another.");
      if (amount > user.balance) return errorResponse(res, 409, "Insufficient funds.");
      await db("users").where({ id}).increment("balance", -amount);
      await db("debits").insert({ owner: id, amount, type: "withdrawal", status: "successful"});
